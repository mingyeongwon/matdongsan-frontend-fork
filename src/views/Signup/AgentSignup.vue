<template>
      ...
      <div class="agentSignup-box border-top ">
        <form v-on:submit.prevent="handleSubmit">
          ...
          <div class="d-flex mb-4 agentProfile-box">
            ...
            <ImagePreview imagePurpose="single" @update:image="handleProfileImageUpdate" class="mt-3 w-100" />
          </div>
          ...
          <div class="d-flex mb-4 agentAddress-box">
            ...
            <div class="agentPassword-box d-flex flex-column">
              <div>
                <input type="text" name="address1" class="address me-2 p-1 ps-2 rounded border border-dark" placeholder="예) 서울시 강남구 봉은사로 1길 12,1층" size="40" v-model="agentSignup.address" readonly />
                <div type="button" class="address-search-btn btn btn-dark btn-sm p-2 mb-1 " @click="openPostSearch">검색</div>
              </div>
              <input type="text" class="row ms-0 mt-3 postcode p-1 ps-2 rounded border border-dark" placeholder="우편번호" size="20" v-model="agentSignup.postcode" readonly />
              <input type="text" class="row ms-0 mt-3 rounded p-1 ps-2 border border-dark" placeholder="상세주소 입력" v-model="agentSignup.addressDetail" size="20" @input="emitUpdate" />
            </div>
          </div>
          ...
          <div class="d-flex mb-5 agentProfile-box">
            ...
            <ImagePreview imagePurpose="single" @update:image="handleDocumentImageUpdate" class="mt-3 w-100" />
          </div>
  ...
  const checkAgentSignupData = computed(() => {
    return agentSignup.value.agentEmail !== "" &&
      ...
      agentSignup.value.documentImage.length > 0; // 서류 이미지가 업로드되었는지 확인
  });
