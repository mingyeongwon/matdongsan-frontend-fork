<template>
          ...
          <div class="col ms-5">
            ...
            <div class="d-flex">
              <input
                ...
                size="40"
                v-model="localPropertyInfo.address"
                readonly
              />
              <div
                type="button"
            ...
            <input
              ...
              placeholder="우편번호"
              size="20"
              v-model="localPropertyInfo.postcode"
              readonly
            />
            <input
              type="text"
              class="row ms-0 mt-3"
              placeholder="상세주소 입력"
              v-model="localPropertyInfo.addressDetail"
              size="20"
              @input="emitUpdate"
            />
          </div>
  ...
  const localPropertyInfo = reactive({ ...props.modelValue });
  
  function openPostSearch() {
    new window.daum.Postcode({
      oncomplete: (data) => {
        localPropertyInfo.postcode = data.zonecode;
        localPropertyInfo.address = data.address;
        emitUpdate();
  ...
  function emitUpdate() {
    emit('update:modelValue', { ...localPropertyInfo });
  }
  
  watch(() => props.modelValue, (newValue) => {
    Object.assign(localPropertyInfo, newValue);
  });
