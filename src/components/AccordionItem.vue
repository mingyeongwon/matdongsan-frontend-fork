    <template>
      <tr @click="rowData.toggle" style="cursor: pointer;">
        ...
        <td class="text-muted" v-if="rowData.item.status == '답변 완료' || rowData.item.status == '처리완료'">
          <small class="bg-success p-2 rounded fw-bold text-light">{{ rowData.item.status }}</small>
        </td>
        <td class="text-muted" v-else>
          <small class="bg-warning p-2 rounded text-dark fw-bold">{{ rowData.item.status }}</small>
        </td>
      </tr>
